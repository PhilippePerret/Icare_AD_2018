...
feature "Commande complete d'un module" do
  scenario "Benoit commande un module qui lui est attribué" do
    ...
    # ---------------------------------------------------------------------
    test 'Phil vient refuser le module d’apprentissage commandé'
    identify_phil
    La feuille a pour titre TITRE_BUREAU
    ...
    motif_refus = "Je ne peux pas vous attribuer ce module."
    Phil remplit le champ 'command[motif_refus]', avec: motif_refus, dans: form_jid
    Phil clique le bouton 'Refuser', dans: form_jid
    La feuille a pour titre TITRE_BUREAU
    ...
    success 'Aucun watcher pour démarré le module n’a été créé pour Benoit.'

    data_mail = {sent_after: start_time, subject: 'Un module vous a été attribué'}
    Benoit ne recoit pas le mail data_mail
    data_mail = {sent_after: start_time, subject: 'Réservation de module refusée', message: [motif_refus]}
