...
=begin
  ...
  @usage
    ...
    FIN
      ...
      La méthode écrit le rapport dans le fichier ./tmp/debug/debug_benchmark.log
      si demandé.
...
class App
  ...
  class Benchmark
    ...
    def logfile
      @logfile ||= begin
        dos = File.join('.','tmp','debug')
        File.exists?(dos) || `mkdir -p "#{dos}"`
        File.join(dos,'debug_benchmark.log')
      end
    end
